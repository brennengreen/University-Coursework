...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (cin.fail())
        {
            ...
            cin.clear();            // to reset the cin status back to "good", so that the further extraction operation can work successfully
	    cin >> input_to_check;
...
void analyzeCoActors(const IMDB& imdb)
{
    ...
    for (auto i = movies_of_actor.begin(); i != movies_of_actor.end(); i++)
    {
        ...
        set<string> actors_in_a_movie = imdb.find_actors_in_a_movie(*i);
